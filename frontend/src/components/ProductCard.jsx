...
const ProductCard = ({ product, hideProfileTypes = false, hideDetails = false }) => {
  ...
  return (
    <Link to={`/product/${product._id}`} className="group h-full">
      <div className={`card hover:shadow-2xl transition-all duration-500 transform hover:-translate-y-3 hover:scale-[1.03] h-full flex flex-col relative overflow-hidden border border-gray-100 dark:border-gray-800 hover:border-primary-200 dark:hover:border-primary-800 ${isApp ? 'p-3' : 'md:p-5'}`}>
        ...
        <div className={`relative overflow-hidden rounded-lg ${isApp ? 'mb-2' : 'mb-3 md:mb-4'} shadow-md group-hover:shadow-xl transition-all duration-500`}>
          <img
            src={
          ...
          />
          <div className={`absolute top-3 left-3 bg-gradient-to-r from-primary-600 via-primary-700 to-primary-800 text-white rounded-full font-bold shadow-xl backdrop-blur-md z-20 border border-white/20 ${isApp ? 'px-2 py-0.5 text-[10px]' : 'px-3 py-1 text-[10px] md:px-4 md:py-1.5 md:text-xs'}`}>
            {product.ottType}
          </div>
        ...
        <h3 className={`font-bold text-gray-900 dark:text-white group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors duration-300 line-clamp-1 ${isApp ? 'text-sm mb-1' : 'text-base mb-2 md:text-xl md:mb-3'}`}>
          {product.name}
        </h3>
        ...
        <div className="flex items-center justify-between mt-auto">
          <div>
            {hideDetails ? (
              <div className="flex flex-col">
                ...
                <span className={`font-extrabold bg-gradient-to-r from-primary-600 to-primary-700 bg-clip-text text-transparent ${isApp ? 'text-xl' : 'text-xl md:text-3xl'}`}>
              ...
              <div className="flex flex-col">
                ...
                <span className={`text-xs text-gray-500 dark:text-gray-400 ${isApp ? '' : 'md:inline'}`}>starting from</span>
              </div>
            )}
          ...
          <Link
            ...
            className={`bg-gradient-to-br from-primary-600 via-primary-700 to-primary-800 hover:from-primary-700 hover:via-primary-800 hover:to-primary-900 text-white rounded-full transition-all duration-300 flex items-center justify-center shadow-xl hover:shadow-2xl transform hover:scale-125 hover:rotate-12 active:scale-95 group-hover:ring-4 ring-primary-200 dark:ring-primary-800 ${isApp ? 'w-10 h-10' : 'w-11 h-11 md:w-12 md:h-12'}`}
            onClick={(e) => e.stopPropagation()}
          >
            <ShoppingCart className={`${isApp ? 'w-4 h-4' : 'w-5 h-5 md:w-6 md:h-6'} group-hover:animate-pulse`} />
          </Link>
        </div>
        </div>
      </div>
